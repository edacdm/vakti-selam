...
export default function NafileNamazlar() {
  ...
  useEffect(() => {
    Animated.loop(
      Animated.timing(mandalaRotation, { toValue: 1, duration: 260000, useNativeDriver: true })
    ).start();
      ...
      <TouchableOpacity style={styles.cardWrapper} onPress={() => router.push(path as any)}>
        <BlurView intensity={25} tint="dark" style={styles.card}>
          ...
          <View style={styles.cardIcon}>
             <MaterialCommunityIcons name="heart-outline" size={30} color={Colors.luxury.gold} />
          </View>
  ...
  return (
    <LinearGradient colors={[Colors.luxury.midnight, Colors.luxury.midnightDeep, "#020406"]} style={styles.container}>
      <Stardust />
      ...
      <Animated.View style={[styles.mandalaLayer, { transform: [{ translateY: mandalaTranslateY }, { rotate: mandalaRotation.interpolate({ inputRange: [0, 1], outputRange: ["0deg", "360deg"] }) }] }]}>
        <ImageBackground source={require("../../../assets/images/mandala_bg.png")} style={styles.mandala} imageStyle={{ opacity: 0.03 }} resizeMode="contain" />
      </Animated.View>

      <Animated.View style={[styles.navHeader, { opacity: headerOpacity }]}>
        <BlurView intensity={30} tint="dark" style={StyleSheet.absoluteFill} />
        <Text style={styles.navTitle}>{t("nafilPrayers" as any)}</Text>
      </Animated.View>
      ...
      <SafeAreaView style={styles.safeArea}>
        <ScrollView contentContainerStyle={styles.scrollContent} showsVerticalScrollIndicator={false} onScroll={Animated.event([{ nativeEvent: { contentOffset: { y: scrollY } } }], { useNativeDriver: false })} scrollEventThrottle={16}>
          ...
          <View style={styles.hero}>
            <Text style={styles.heroTitle}>{t("nafilPrayers" as any)}</Text>
            <Text style={styles.heroSub}>{t("nafilPrayersDesc" as any)}</Text>
            <View style={styles.heroDivider} />
          </View>

          <BlurView intensity={30} tint="light" style={styles.quoteCard}>
            <Text style={styles.quoteText}>{t("menuNafileQuote" as any)}</Text>
          </BlurView>

          <View style={styles.grid}>
            {renderPrayerCard(t("namazTeheccud" as any), t("namazTeheccudDesc" as any), "/namazlar/NafileNamazlar/Teheccud", 0)}
            {renderPrayerCard(t("namazKusluk" as any), t("namazKuslukDesc" as any), "/namazlar/NafileNamazlar/Kusluk", 1)}
            {renderPrayerCard(t("namazEvvabin" as any), t("namazEvvabinDesc" as any), "/namazlar/NafileNamazlar/Evvabin", 2)}
            {renderPrayerCard(t("namazTeravih" as any), t("namazTeravihDesc" as any), "/namazlar/NafileNamazlar/Teravih", 3)}
          </View>
...
const styles = StyleSheet.create({
  ...
  quoteCard: { borderRadius: 24, padding: 18, marginBottom: 24, borderWidth: 1, borderColor: "rgba(212, 175, 55, 0.3)", overflow: "hidden" },
