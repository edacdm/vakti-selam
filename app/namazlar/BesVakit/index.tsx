    ...
    particles.forEach(p => {
      const animate = () => {
        ...
        Animated.parallel([
          ...
          Animated.timing(p.scale, { toValue: 1, duration: p.duration, useNativeDriver: true })
        ]).start(() => animate());
      };
      animate();
    });
...
export default function BesVakit() {
  ...
  const renderPrayerCard = (title: string, desc: string, path: string, index: number) => (
    <Animated.View style={{ 
      ...
      transform: [
        { translateX: fadeAnims[index].interpolate({ inputRange: [0, 1], outputRange: [20, 0] }) },
        { scale: fadeAnims[index].interpolate({ inputRange: [0, 1], outputRange: [0.95, 1] }) }
      ]
    }}>
      <TouchableOpacity 
      ...
      >
        <BlurView intensity={25} tint="dark" style={styles.card}>
          ...
          <View style={styles.cardIcon}>
             <MaterialCommunityIcons name="clock-check-outline" size={28} color={Colors.luxury.gold} />
          </View>
          <View style={styles.cardText}>
            <Text style={styles.cardTitle}>{title}</Text>
            <Text style={styles.cardDesc}>{desc}</Text>
          </View>
          <Ionicons name="chevron-forward" size={20} color={Colors.luxury.gold} />
        </BlurView>
      </TouchableOpacity>
    </Animated.View>
  );
  ...
  return (
    <LinearGradient colors={[Colors.luxury.midnight, Colors.luxury.midnightDeep, "#050812"]} style={styles.container}>
      <Stardust />
      ...
      <Animated.View style={[styles.navHeader, { opacity: headerOpacity }]}>
        ...
        <Text style={styles.navTitle}>{t("fiveDailyPrayers")}</Text>
      </Animated.View>
      ...
      <SafeAreaView style={styles.safeArea}>
        ...
        >
          ...
          <View style={styles.hero}>
            <Text style={styles.heroTitle}>{t("fiveDailyPrayers")}</Text>
            <Text style={styles.heroSub}>{t("fiveDailyPrayersDesc")}</Text>
          ...
          <View style={styles.grid}>
            {renderPrayerCard(t("namazSabah"), t("namazSabahDesc"), "/namazlar/BesVakit/SabahNamazi", 0)}
            {renderPrayerCard(t("namazOgle"), t("namazOgleDesc"), "/namazlar/BesVakit/SabahNamazi", 1)}
            {renderPrayerCard(t("namazIkindi"), t("namazIkindiDesc"), "/namazlar/BesVakit/SabahNamazi", 2)}
            {renderPrayerCard(t("namazAksam"), t("namazAksamDesc"), "/namazlar/BesVakit/SabahNamazi", 3)}
            {renderPrayerCard(t("namazYatsi"), t("namazYatsiDesc"), "/namazlar/BesVakit/SabahNamazi", 4)}
          </View>
